...
describe('defaults', () => {
  ...
  it('encodes the defaults it claims', () => {
    ...
    expect(byCommand.get('goto_split:up')).toBe('alt+cmd+up')
    expect(byCommand.get('goto_split:down')).toBe('alt+cmd+down')
    expect(byCommand.get('goto_split:next')).toBe('cmd+]')
    expect(byCommand.get('goto_split:previous')).toBe('cmd+[')
